...
- 桌面程序现在支持两种导入方式：
- 命令行：`AiSmartDrill.App.exe --import "xxx.skillpkg" --mode direct`
- 自定义协议：`aismartdrill://launch?payload=...`

首次正常启动桌面程序后，会自动在当前 Windows 用户下注册 `aismartdrill://` 协议。之后从网页点击“网页唤起本地刷题”时，浏览器会把技能包 payload 交给本程序；程序会先把 payload 落成临时 `.skillpkg` 文件，再复用现有的 CareerPath 导入与单实例 IPC 流程。
